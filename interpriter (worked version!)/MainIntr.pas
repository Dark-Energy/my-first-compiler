...
(* --------------- СКАННЕР ------------------- *)
 ...
 var
   Buffer:PChar; // буфер
   BufPtr:PChar; //указатель в буфере
   BufSize:longint;//размер буфера
   TokenPtr:PChar;//указатель начала текущей лексемы
   TokenEnd:PChar;//указатель конца лексемы
   TokenValue:TTokenType; //значение лексемы
   TokenStr:array[0..255]of char;//строка - вид лексемы
   Ident:PSymboll;//идентификатор
   LitType:integer;//тип литерала
   LenString:integer;//длина строки-лексемы
   isFunc:boolean; //внутри функции
   CurFunc:PUsDef;//текущая функция
...
begin
  ...
  TokenValue:= ttEof;
  TokenPtr := BufPtr;
  if BufPtr^ = #0 then exit;
  if Bufptr^ in Alpha then begin
    while Bufptr^ in AlNum do inc(bufptr);
    TokenEnd:= Bufptr;
    TokenLen := TokenEnd - TokenPtr;
    if TokenLen > 255 then TokenLen:= 255;
    System.Move(TokenPtr^, TokenStr[0], TokenLen);
    StrUpper(TokenStr);
    TokenValue:= ttName;
    case TokenStr[0] of 
     'A': if strcomp(TokenStr, 'ARRAY') then TokenValue:= ttArray
       else if strcomp(TokenStr, 'AND') then TokenValue:= ttAnd;
     'B': if strcomp(TokenStr, 'BEGIN') then TokenValue := ttBegin;
     'C': if strcomp(TokenStr, 'CONTINUE') then TokenValue := ttContinue;
     'D': if strcomp(TokenStr, 'DEFAULT') then TokenValue:= ttDefault
       else if strComp(TokenStr, 'DIV') then TokenValue := ttDiv
       else if strcomp(TokenStr, 'DO') then TokenValue:= ttDo
       else if StrComp(TokenStr, 'DEFUN') then TokenValue := ttDefun; 
     'E': if StrComp(TokenStr, 'END') then TokenValue := ttEnd
       else if StrComp(TokenStr, 'ELSE') then TokenValue := ttElse
       else if Strcomp(TokenStr, 'ENDIF') then TokenValue := ttEndIf   
       else if StrComp(TokenStr, 'ENDSEL') then TokenValue:= ttEndSel;
     'F': if StrComp(TokenStr, 'FALSE') then TokenValue:= ttFalse
       else if StrComp(TokenStr, 'FOR') then TokenValue := ttFor;
     'I': if StrComp(TokenStr, 'IF') then TokenValue := ttIf;
     'M': if StrComp(TokenStr, 'MOD') then TokenValue := ttMod;
     'N': if StrComp(TokenStr, 'NOT') then TokenValue:= ttLNot
       else if StrComp(TokenStr, 'NEXT') then TokenValue := ttNext;
     'O': if StrComp(TokenStr, 'OR') then TokenValue := ttLOr;
     'R': if StrComp(TokenStr, 'REPEAT') then TokenValue := ttRepeat
       else if StrComp(TokenStr, 'RETURN') then TokenValue := ttReturn;
     'S': if StrComp(TokenStr, 'SELECT') then TokenValue := ttSelect
       else if StrComp(TokenStr, 'STEP') then TokenValue := ttStep
       else if StrComp(TokenStr, 'SHL') then TokenValue := ttShl
       else if StrComp(TokenStr, 'SHR') then TokenValue := ttShr;
     'T': if StrComp(TokenStr, 'THEN') then TokenValue := ttThen
       else if StrComp(TokenStr, 'TRUE') then TokenValue := ttTrue
       else if StrComp(TokenStr, 'TO') then TokenValue := ttTo;
     'U': if StrComp(TokenStr, 'UNTIL') then TokenValue := ttUntil;
     'V': if StrComp(TokenStr, 'VAR') then TokenValue := ttVar; 
     'W': if StrComp(TokenStr, 'WHILE') then TokenValue := ttWhile
       else if StrComp(TokenStr, 'WEND') then TokenValue := ttWend;
     'X': if StrComp(TokenStr, 'XOR') then TokenValue := ttLXor; 
    end;
  end
  else if Bufptr^ in Digits then begin
        ...
        c:= Bufptr[0]; inc(Bufptr);
        case C of
            '*': if Bufptr[0] = '*' then 
                   if Bufptr[1] = '=' then TokenValue := ttPowAsg 
                   else TokenValue:= ttPow
                 else TokenValue:= ttMul; 
            '|':
                
               if Bufptr[0] = '|' then TokenValue:= ttLor
               else if Bufptr[0] = '=' then TokenValue := ttOrAsg
               else TokenValue:= ttOr;
            '&':
                begin
               inc(BufPtr);
               if BufPtr^ = '&' then
            ...
            '!': if Bufptr[0] = '=' then TokenValue:= ttNe else TokenValue:= ttLNot;
            '=': if Bufptr[0] = '>' then TokenValue:= ttGe else TokenValue:= ttEq;
            ':': if Bufptr[0] = '=' then TokenValue:= ttAssign;
            '+': if Bufptr[0] = '+' then TokenValue:= ttInc else TokenValue:= ttAdd;
            '-': if Bufptr[0] = '-' then TokenValue:= ttDec else TokenValue:= ttSub;
          end;
          if TokenValue in [ttInc, ttDec, ttShl, ttShr, ttLor, ttNe 
            '"':
              begin
                inc(BufPtr);
...
const
  ...
  (Name: 'RANDOV'; Func:(addr: @ARandom; Ftype: ftFunc; count:1; List:'')),
...
begin
  try
    ...
    try
      ...
      ShowMessage('ошибка создания таблицы символов');
      raise;
    end;
    ...
    try
      ...
      ShowMessage('Ошибка загрузки ключевых слов');
      raise ;
    end;
    ...
    try
      for i:= 1 to FuncCount do begin
        try
          ...
          ShowMessage('Неверное имя функции');
          raise ;
        end;
        if s = nil then raise EInvalidPointer.Create(
          'fuck the optimization');
        s^.func:= @FuncTable[i].func;
      end;
    except
      on EInvalidPointer do begin
        ShowMessage('Неверный указатель функции');
        raise;
      end
      else
        ShowMessage('Ошибка загрузки функций');
      raise;
    end;
